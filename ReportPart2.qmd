...
for category in categories:
  ...
  # Append TS to list
  cat_covars.append(covars)
  
# Cleanup
del covars
```

```{python CategoryStaticCovars}

# Create dataframe where column=static covariate and index=category
cat_static = df["category"].reset_index().drop("date", axis=1).drop_duplicates().set_index("category")
cat_static["category"] = df["category"].unique()

# Encode static covariates
cat_static = pd.get_dummies(cat_static, sparse = True, drop_first = True)
...
for series in category_sales:
  ...
  series = series.with_static_covariates(
    cat_static[cat_static.index == series.components[0]]
    )
...
# Naive drift
model_drift.fit(ts_sales[categories][:-15])
pred_drift_cat = model_drift.predict(n = 15)

# Naive seasonal
model_seasonal.fit(ts_sales[categories][:-15])
pred_seasonal_cat = model_seasonal.predict(n = 15)


# ETS

# First fit & validate the first category to initialize series
model_ets.fit(y_train_cat[0])
pred_ets_cat = model_ets.predict(n=15)

# Then loop over all categories except first
for i in tqdm(range(1, len(y_train_cat))):

  # Fit on training data
  model_ets.fit(y_train_cat[i])

  # Predict validation data
  pred = model_ets.predict(n=15)

  # Stack predictions to multivariate series
  pred_ets_cat = pred_ets_cat.stack(pred)
...
for series in y_train_cat:
  ...
  trend, seasonality = decomposition(
    ...
    trend = 731, # Size of trend smoother (last n lags)
    robust = True
  )
...
pred_linear_cat = model_linear.predict(
  n=15,
  future_covariates = x_cat[0][linear_covars]
  )
...
pred_arima_cat = model_arima.predict(
  n=15,
  future_covariates = x_cat[0][arima_covars])
...
```{python CatDLinearFit}

# All covariates
dlinear2_covars = ['tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'day_sin', 'day_cos', "month_sin", "month_cos", 'oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'transactions', 'trns_ma7', 'local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday']

# Fit DLinear model
model_dlinear.fit(
  series = [y[:-45] for y in y_train_cat],
  future_covariates = [x[dlinear2_covars] for x in x_cat],
  val_series = [y[-45:] for y in y_train_cat],
  val_future_covariates = [x[dlinear2_covars] for x in x_cat],
  verbose = True
)
...
```{python CatDLinearLoad}

# Load a previously trained DLinear model's best checkpoint
model_dlinear = DLinear.load_from_checkpoint("DLinearCat2.0", best = True)

```

```{python CatDLinearVal}

# First fit & validate the first store to initialize series
pred_dlinear_cat = model_dlinear.predict(
  n=15,
  series = y_train_cat[0],
  future_covariates = x_cat[0][dlinear2_covars]
  )
...
for i in tqdm(range(1, len(y_train_cat))):
  ...
  pred = model_dlinear.predict(
    n=15,
    series = y_train_cat[i],
    future_covariates = x_cat[i][dlinear2_covars]
  )

  # Stack predictions to multivariate series
  pred_dlinear_cat = pred_dlinear_cat.stack(pred)
...
# Naive drift
scores_hierarchy(
  ts_sales["AUTOMOTIVE":"SEAFOOD"][-15:],
  trafo_zero(pred_drift_cat),
  categories, 
  "Naive drift"
  )
...
```{python CategoryScorePlot}

cat_preds_dict = {
  # "Naive drift": trafo_zero(pred_drift_cat),
  # "Naive seasonal": trafo_zero(pred_seasonal_cat),
  "ETS": trafo_zero(pred_ets_cat),
  "Linear + AutoARIMA": trafo_zero(pred_linear_cat + pred_arima_cat),
  "Linear + Random forest": trafo_zero(pred_linear_cat + pred_rf_cat)
}
