...
total_covars["local_holiday"] = total_covars1["local_holiday"]
total_covars["regional_holiday"] = total_covars1["regional_holiday"]
...
def scores_plot(val, preds_dict, subset, logscale = True):
...
print("Total sales prediction scores, baseline models")
print("--------")
...
pred_total = model_linear.predict(
  n = 15, 
  future_covariates = x_decomp[['oil_ma28', 'onp_ma28']],
  past_covariates = x_decomp[['sales_ema5', 'trns_ma7']]
  ) + ts_preds_total1[-15:]
...
early_stopper = EarlyStopping(
  monitor = "val_loss",
  min_delta = 10292685, # 0.1% of MSE of best baseline
  patience = 10
)
...
dlinear_futcovs_total = ['national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'oil', 'oil_ma28', 'local_holiday', 'regional_holiday', 'onpromotion', 'onp_ma28', 'day_sin', 'day_cos', 'month_sin', 'month_cos']
...
_ = model_ets.fit(y_train_store[0])
...
for i in tqdm(range(1, len(y_train_store))):
  ...
  _ = model_ets.fit(y_train_store[i])
...
# Time covariates (trend + seasonality, future only)
...
#### Linear regression on remainder
...
# Covariates (cylical + calendar, future & past)
...
rf_futcovars = ['oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday']

rf_pastcovars = ["sales_ema7", 'transactions', 'trns_ma7']
...
# Time covariates (trend + seasonality, all future)
...
rnn_covars = ['oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday']
...
store_preds_dict = {
  ...
  "Exponential smoothing": trafo_zeroclip(pred_ets_store),
  "D-Linear + RNN": trafo_zeroclip(pred_dlinear2_store + pred_rnn_store),
  "D-Linear (all features)": trafo_zeroclip(pred_dlinear2_store)
...
_ = model_ets.fit(y_train_disagg[0])
...
for i in tqdm(range(1, len(y_train_disagg))):
  ...
  # Fit on training data
  _ = model_ets.fit(y_train_disagg[i])
...
# Covariates (all, past same as store linear 2)
linear_disagg_futcovars = ["trend", 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'day_sin', 'day_cos', "month_sin", "month_cos", 'oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday']
...
Model = D-Linear (global, all features)
