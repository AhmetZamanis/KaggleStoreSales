...
for store in [int(store) for store in stores]:
  ...
  # Retrieve static covariates as column: covariate, index: store

  # Create darts TS, fill gaps
  covars = TimeSeries.from_dataframe(
    covars,
    static_covariates = static_covars,
    freq = "D"
    ) 
  ...
  covars = trafo_static.fit_transform(covars)
...
grid search runtime: 9-10mins
grid search best tune and score:
...
pred_total = model_xgb.predict(
  n = 227, future_covariates = x_total) + ts_preds_total1[-243:-16]
...
Try same approach as category, + consider global models for decomposed store sales
