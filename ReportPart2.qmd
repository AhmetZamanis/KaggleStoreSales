...
# AutoARIMA
arima_covars = ['local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2', 'ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'transactions', 'trns_ma7', 'day_sin', 'day_cos']
...
```{python CategoryDLinearVal}

# # Predict validation data with RNN model
# pred_dlinear_cat_list = model_dlinear_cat.predict(
#   n = 227,
#   series = y_train_cat,
#   future_covariates = [x[dlinear_covars] for x in x_cat]
#   )
...
# pred_dlinear_cat = pred_dlinear_cat_list[0].stack(pred_dlinear_cat_list[1])
# for pred in pred_dlinear_cat_list[2:]:
...
```{python CategoryRNNVal}

# # Predict validation data with RNN model
# pred_rnn_cat_list = model_rnn_cat.predict(
#   n = 227,
#   series = y_train_cat,
#   future_covariates = [x[rnn_covars] for x in x_cat]
#   )
#   
# # Stack predictions to get multivariate series
# pred_rnn_cat = pred_rnn_cat_list[0].stack(pred_rnn_cat_list[1])
# for pred in pred_rnn_cat_list[2:]:
#   pred_rnn_cat = pred_rnn_cat.stack(pred)
```
...
num_loader_workers causes training / sanity check to get stuck. it works without num_loader_workers, but uses single CPU core for data loader and accuracy calculation. It's possible this is because of jupyter notebook

default progress bar prevents RNN checkpoint saving, it works with rich progress bar, but it doesn't print epochs default progress bar prints epochs but freezes rstudio after a while.

Dlinear sometimes doesn't train because "tensor a doesn't match tensor b". tensor a is input chunk, tensor b is 1 smaller than that.

Overwriting a darts NN model with a new model name seems to cause the same pickle issue as default progress bar.

can't suppress scientific notation with torch_setoptions

**Params:**

RNN loss is computed on output sequences of training length - input_chunk_length. Try longer input lengths

Use an activation that forces \>=0 predictions?

Change learning rate / use adaptive rate / different optimizer?

Set much smaller early stop value?
