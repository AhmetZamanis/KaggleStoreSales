...
`{# {python TotalSalesXGB} # from darts.models.forecasting.xgboost import XGBModel #  # # Hyperparameter search space # xgb_params = { #   "lags": [[-1, -2, -3, -4, -5]], #   "lags_future_covariates": [[0]], #   "n_jobs": [-2], #   "learning_rate": [0.1, 0.2, 0.3], #   "max_depth": [4, 6, 8], #   "min_child_weight": [1, 4, 8], #   "gamma": [0, 0.05, 0.1], #   "subsample": [0.6, 0.8, 1], #   "colsample_bytree": [0.6, 0.8, 1], #   "n_estimators": [100, 250, 500] # } #  # # Decomposed validation series # y_val_total_decomp = y_val_total - ts_preds_total1[-243:-16] #  # # Full covariates series # x_total = x_train_total.append(x_val_total) #  # # Grid search # model_xgb, xgb_besttunes, xgb_bestscore = XGBModel.gridsearch( #   parameters = xgb_params, #   series = y_train_total, #   # forecast_horizon = 15, #   # start = 0.75, #   # stride = 15, #   future_covariates = x_total, #   val_series = y_val_total_decomp, #   metric = rmse # )`
...
`{# {python TotalSalesXGBScore} #  # # Fit model & predict validation set # model_xgb.fit(y_train_total, future_covariates = x_total) # pred_total = model_xgb.predict( #   n = 227, future_covariates = x_total) + ts_preds_total1[-243:-16] #  # # Score model # perf_scores(y_val_total, pred_total, model="Linear + XGB hybrid, total sales")`
...
# Predict validation data with RNN
pred_rnn_store_list = model_rnn_store.predict(
  n = 227,
...
pred_rnn_store = pred_rnn_store_list[0].stack(pred_rnn_store_list[1])
for pred in pred_rnn_store_list[2:]:
...
```
    ...
    MAE: mean = 4280.72, sd = 13577.98, min = 832.71, max = 102655.97
    MSE: mean = 720356988.79, sd = 5147081658.91, min = 1292235.24, max = 38191501850.13
    RMSE: mean = 6760.91, sd = 25973.97, min = 1136.77, max = 195426.46
    RMSLE: mean = 0.75, sd = 0.45, min = 0.61, max = 4.03
    --------
