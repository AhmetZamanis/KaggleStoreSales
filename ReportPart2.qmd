...
### Multivariate, hybrid: Linear + XGBoost

#### Time decomposition with linear regression
...
# Time covariates
linear_covars = ["trend", 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'day_sin', 'day_cos']
...
# Score decomposition model predictions
scores_hierarchy(
  ts_sales["AUTOMOTIVE":"SEAFOOD"][-227:],
  pred_linear_cat,
  categories,
...
# First retrieve residuals for first category to initialize series
res_linear_cat = model_linear_cat.residuals(
  y_train_cat[0],
  future_covariates = x_cat[0][linear_covars]
  )
...
for i in tqdm(range(1, len(y_train_cat))):
  ...
  # Stack residuals to multivariate series
  res_linear_cat = res_linear_cat.stack(res)
```

```{python CatLinearResidsSplit}

# Drop residuals before 2014
decomp_cat = res_linear_cat.split_after(pd.Timestamp("2013-12-31"))[1]
```

#### XGBoost on decomposed residuals
...
# XGB covariates
xgb_covars = ['local_holiday', 'regional_holiday', 'national_holiday', 'ny1', 'ny2','ny_eve31', 'ny_eve30', 'xmas_before', 'xmas_after', 'quake_after', 'dia_madre', 'futbol', 'black_friday', 'cyber_monday', 'oil', 'oil_ma28', 'onpromotion', 'onp_ma28', 'transactions', 'trns_ma7']
...
```{python HybridXGBPreds}
# Score hybrid linear + XGB model predictions
scores_hierarchy(
  ts_sales["AUTOMOTIVE":"SEAFOOD"][-227:],
  (pred_linear_cat + pred_xgb_cat).map(lambda x: np.clip(x, a_min = 0, a_max = None)),
  categories,
  "Linear + XGBoost"
  )
```
...
scores_hierarchy(
  ts_sales["AUTOMOTIVE":"SEAFOOD"][-227:],
  pred_arima_cat.map(lambda x: np.clip(x, a_min = 0, a_max = None)),
  categories,
  "AutoARIMA"
  )
```
