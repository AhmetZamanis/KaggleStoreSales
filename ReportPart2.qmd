...
for series in y_train_cat:
  ...
  trend, seasonality = decomposition(
    ...
    trend = 365, # Size of trend smoother (last n lags)
    robust = True
  )
...
cat_preds_dict = {
  ...
  "Linear": trafo_zero(pred_linear_cat),
...
##### Validation set: Remainder of linear model predictions
...
# # Load a previously trained RNN model
# model_rnn_store = model_rnn1.load_from_checkpoint("RNNStore1.0", best = True)
...
# First fit & validate the first store to initialize series
pred_rnn1_store = model_rnn1.predict(
  n=15,
...
for i in tqdm(range(1, len(y_train_store))):
  ...
  # Stack predictions to multivariate series
  pred_rnn1_store = pred_rnn1_store.stack(pred)
  
del pred, i
```

##### Validation set: Remainder of D-Linear predictions
...
# scores_hierarchy(
#   ts_sales[stores][-15:],
#   trafo_zero(pred_linear_store + pred_rnn1_store),
#   stores,
#   "Linear + RNN (global)"
#   )
...
store_preds_dict = {
  ...
  "Linear": trafo_zero(pred_linear_store),
  "Linear + AutoARIMA": trafo_zero(pred_linear_store + pred_arima_store),
  "Linear + Random forest": trafo_zero(pred_linear_store + pred_rf_store)
  # "Linear + RNN": trafo_zero(pred_linear_store + pred_rnn1_store)
}
