...
# Then loop over all categories except first
for i in tqdm(range(1, len(res_linear_cat))):
...
```
    ...
    Model = Linear (time features only)
...
The residuals retrieved from darts are likely reducing performance of hybrid models.

Remaining options:

-   STL decomposition of entire series in darts (will have to extrapolate trend & seasonality for test predictions)

-   Linear decomposition in sklearn, or historical_forecasts with retrain=False? Open github issue?

-   Decomp + ARIMA hybrid (time consuming, likely won't do much better)

-   Tune XGBoost (likely not very time consuming, likely won't do much better if at all)
