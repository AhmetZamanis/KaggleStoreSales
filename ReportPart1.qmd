...
-   The ETS model also beats FFT in RMSE and RMSLE, while having considerably higher MAPE.

-   This is likely because MAPE is a measure of relative error, while RMSE and RMSLE are measures of absolute error.
...
-   In contrast, the linear model's trend and seasonality are both on point, and the January 1st drop is adjusted for nicely. The model is not able to match some peaks and troughs fully, which are possibly cyclical in nature. That's where model 2 will come in.
...
-   Combining model 1 with a second linear model improves our predictions a bit, especially the MAPE.

    -   This means absolute errors aren't reduced as much as relative errors are.
...
Before carrying on with our best hybrid model, which is linear + random forest, we can view the model summary of the AutoARIMA process for some insight.

The AutoARIMA process has chosen an ARIMA(2, 0, 1) model, which uses the last 2 lags and the last forecast error as predictors.
...
-   We see that the first lag has a positive coefficient, so a higher value for lag 1 means a higher prediction for sales at T=0.

-   Lag 2 and forecast error of lag 1 have a negative effect on the prediction. If lag 2 or the error of lag 1 increase, the prediction for T=0 is pulled back.

-   The p-value for the Ljung-Box test is 0.79, so the null hypothesis of residual independence is accepted. The residuals of our model are not correlated, so applying a third autoregressive model on them is not likely to improve predictions.

-   The p-value for the heteroskedasticity test is 0.88, so the residuals meet the equal variance assumption. The model makes similar errors across the entire predicted time series, which means its performance is consistent.

-   The null hypothesis for the Jarque-Bera test is zero skew & zero excess kurtosis in the distribution of the tested data. With a p-value of 0, the null hypothesis is rejected, and we can see the residuals have a skew of 0.81 and kurtosis of 9.28, compared to 0 and 3 respectively for a normal distribution.
...
-   The distribution of residuals is more centered around 0 (as the AutoARIMA summary suggested), with smaller extreme values. This time, the extreme values are well-balanced between positive and negative. It seems the model underpredicts a bit more than it overpredicts. The opposite would be more preferable.
...
-   The p-value for the KPSS test is larger than 0.05 (although barely), leading us to accept the null hypothesis of stationarity around a constant.
