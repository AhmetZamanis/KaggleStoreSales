...
-   However, we performed time decomposition on sales, removing seasonal effects.

-   Our covariate series may display their own seasonalities. We can use stationarity tests to quickly check for this.
...
Now our columns all represent the change in each covariate compared to the previous period.

### Sales features

Let's evaluate features from sales lags as a possible predictor.
...
As we saw in model 1 residuals analysis, sales lag 1 makes a strong contribution to the partial autocorrelation, while lags 2 to 7 make weak but significant contributions.
...
It doesn't look like any lag after 1 displays a different relationship with sales.
...
We can see sales EMA7 is a good linear predictor of sales, similar to lag 1. Let's compare its correlation with sales to lag 1's correlation with sales.
...
-   Because of these, we'll just consider moving averages of the oil prices as features.

-   These won't be exponentially weighted averages, as recent oil prices likely require time to make their effect on sales.
...
We don't see particularly strong relationships between oil price change MAs and sales, though the monthly MA is the most notable one, so we will use that as a feature.

Since we need 28 past periods to calculate this MA for a 29th period, we'll have missing values for the first 28 periods in our data. After some iteration, these were interpolated well with a second-order spline interpolation.
