...
Our main candidate for the time decomposition model is a simple linear regression, but with numerous time features. We'll flag the dates that influence sales considerably with dummy or ordinal features.

### Calendar effects features

Sales decline very sharply, almost to zero in January 1st every year. The sales in January 2nd recover sharply, which is a relatively huge increase from the 1st, so we will flag both dates.
...
-   For Christmas effects, we will create two integer columns that reflect the strength of the Christmas sales increase, based on day of month. For dates outside December 13-27, these features will take a value of zero. This may not be the most sophisticated way to reflect the Christmas effects on our model, but it should be simple and effective.
...
To account for the effect of the April 2016 earthquake, we create a feature that reflects the strength of the increase in sales based on the date. The feature takes a value of zero for dates outside April 17-22, 2016.
...
We previously created dummy features to indicate local-regional-national holidays, and special events. There are only a few different events, and they differ in nature, so we will create separate dummy features for each one. We already created a feature for the earthquake, so we'll skip that one.
...
To capture the weekly seasonality in a simple manner, we'll create a dummy feature for each day of the week. We won't create one for Monday to avoid perfect multicollinearity.
...
Now we will aggregate the time features by mean. For local and regional holidays, this will give us values between 0 and 1, reflecting the
